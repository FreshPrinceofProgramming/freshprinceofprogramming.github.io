...
The way that a ceasar cipher works is through a series of shifts or substitutions of the alphabet to a fixed position. In other words, all you're doing is shifting the alphabet by a certain number of characters to come up with the ciphertext. For example, if we wanted to encrypt the plaintext, "Attack," here's what we would do:
...
2. Now let's take our phrase "Attack" and encrypt it using the shift. The result is "Dwwdfn." We now have our corresponding ciphertext

3. Done

That was very simple! The shift of 3 gave us a completely different string that can't immediately be deciphered as "Attack." The shift of 3 is also what I used in the subject title. If we work backwards, the corresponding plaintext value, "Can you read this?", is the result. As trivial as this encryption method is, history shows this is what Julius Ceasar was using to send messages to his military. Only the correct people knew the shift value and could decrypt the message appropriately. Here's a visual representation of what the substitutions may look like:

![ceasar]({{ site.url }}{{ site.baseurl }}/assets/images/ceasar.jpg)

Cool! So while being able to use and understand a ceasear ciper is great, it's nowhere near the level of appropriate encryption needed today to secure data. 
